...
function adiciona_seo_personalizado($post_id) {
    if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) return;

    global $wpdb;    
       
    if(get_post_type($post_id) == "perfil"):

        $post = get_post($post_id); 
        $slug = $post->post_name;
        $id_anunciante_ex = explode('-', $slug);
        $id_anunciante = $id_anunciante_ex[0];

        $resultado = $wpdb->get_results('
            Select
            estado, cidade, bairro, sexo, idade, descricao 
            FROM 
            wp_ht_anunciante
            WHERE user_id = '.$id_anunciante.'', ARRAY_A);

        $user = get_user_by('ID', $id_anunciante);
        $user_name = $user->display_name;
        if(!empty($resultado)):
            $meta_value = $user_name.' - Acompanhante em '.$resultado[0]['cidade'].'-'.$resultado[0]['estado'].' - Scortsbook';
            update_post_meta( $post_id, '_yoast_wpseo_title', $meta_value);
            update_post_meta( $post_id, '_yoast_wpseo_metadesc', $meta_value);
        endif;
    else:
        return false;
    endif;
...
add_action( 'save_post', 'adiciona_seo_personalizado', 20, 2 );
//add_filter( 'pre_post_update', 'atualiza_seo', 10, 3 );

function atualiza_seo($post_id)
{
    update_post_meta($post_id, '_yoast_wpseo_metadesc', 'teste_funcao');
}